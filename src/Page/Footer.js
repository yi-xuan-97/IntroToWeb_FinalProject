...
//
// Note: lucide-react removed brand icons (Github, Linkedin) in recent versions
// for trademark reasons. We use inline SVG for those, and Mail from lucide.
import React from "react";
import { Mail } from "lucide-react";

// Inline brand SVGs — official paths from simple-icons / brand resources.
// Inheriting `currentColor` so they pick up Tailwind's text color.
  ...
  return (
    <footer className="bg-emerald-800 text-stone-100">
      <div className="max-w-3xl mx-auto px-5 sm:px-6 py-12 sm:py-14 text-center">
        ...
        <p className="mt-2 text-sm sm:text-base text-emerald-100/75">
          Looking forward to hearing from you.
        </p>

        <div className="mt-7 flex items-center justify-center gap-7">
          <a
            href="https://www.linkedin.com/in/yixuan-feng/"
            target="_blank"
            rel="noopener noreferrer"
            className="text-emerald-100/80 hover:text-white
                       transition-colors duration-200
                       focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-200/50 rounded"
            aria-label="LinkedIn"
          >
            <LinkedinIcon className="w-6 h-6" />
          </a>
          <a
            href="https://github.com/yi-xuan-97/"
            target="_blank"
            rel="noopener noreferrer"
            className="text-emerald-100/80 hover:text-white
                       transition-colors duration-200
        ...
        <p className="mt-9 text-xs text-emerald-200/55 tracking-wide">
          © 2026 Erica Feng &nbsp;·&nbsp; Built with React, Tailwind, and a
          little patience.
        </p>
      </div>
    </footer>
